...
core_Index = 36.6 < x & x < 44.4;
back_Index = 44.5 < x & x < 50;
zdr_network = zdr;
zdr_network(back_Index) = zdr_network(back_Index) + 8;
%zdr_network(core_Index) = zdr_network(core_Index) + (x(core_Index)' - 36.5);
%zdr_network(core_Index) = zdr_network(core_Index) + -2/16*(x(core_Index)').^2 + 178/16*x(core_Index)' - 239.531;
...
L = sum(core_Index);
for i = 2:L
    A_kdp(i) = trapz(X(1:i), Kdp(1:i))*trapz(X(1:i), Z(1:i));
    A_dbz(i) = trapz(X(1:i), Z(1:i));
end

%trapz(X(1:i), Kdp(1:i))*trapz(X(1:i), Z(1:i));

kpd_weight = 0.2;

A = 8*(kpd_weight*A_kdp/A_kdp(L) + (1-kpd_weight)*A_dbz/A_dbz(L));

% figure(2)
% plot(X,A)

zdr_network(core_Index) = zdr_network(core_Index);% + A';
