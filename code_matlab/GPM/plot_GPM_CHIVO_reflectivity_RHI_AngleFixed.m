...
ZcKu_RHI = ZcKu(70:176, swath_number, Lat_lon_index);
[r,~,c] = size(ZcKu_RHI);
...
pcolor(lat_rhi,z,squeeze(ZcKu_RHI));
hold on 
...
Lat_lon_index = -33 < lat_swath & lat_swath < -31.5 & -65 < lon_swath & lon_swath < -63;
ZcKa_RHI = ZcKa(70:176, swath_number - 12, Lat_lon_index);
[r,~,c] = size(ZcKa_RHI);
...
pcolor(lat_rhi,z,squeeze(ZcKa_RHI));
hold on 
plot(-31.63*ones(1,r),z,'k--','linewidth',1.5);
hold off
...
pcolor(lat_rhi,z,squeeze(diff_freq_ratio));
hold on 
plot(-31.63*ones(1,r),z,'k--','linewidth',1.5);
hold off
