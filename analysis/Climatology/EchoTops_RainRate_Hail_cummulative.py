...
RainRate_Accummulation = np.zeros([801,801])
Hail_Accumulation = np.zeros([801,801])
EchoTop_14 = np.zeros([801,801])
EchoTop_18 = np.zeros([801,801])
...
for file_index in range(len(chivo_files)):
    ...
    # RainRate_combined.filled(0)
    # RainRate_combined.mask = False
    
    # RainRate_Accummulation = RainRate_Accummulation + RainRate_combined/2
    ...
    EchoTop_combined.filled(0)
    EchoTop_combined_mask = False

    EchoTop18_array = np.zeros([801, 801])
    EchoTop14_array = np.zeros([801, 801])
    
    for i in  range(len(EchoTop_combined)):
        for j in range (len(EchoTop_combined[i])):
            if EchoTop_combined[i][j] > 18:
                EchoTop18_array[i][j] = 1
            
            if EchoTop_combined[i][j] > 14:
                EchoTop14_array[i][j] = 1
    
    EchoTop_18 = EchoTop_18 + EchoTop18_array
    EchoTop_14 = EchoTop_14 + EchoTop14_array
    
    print('done with echotops ' + str(file_index))
    ...
    # hail_detection.filled(0)
    # hail_detection.mask = False
    # Hail_Accumulation = Hail_Accumulation + hail_detection 
    
    # print('done with hail ' + str(file_index))
...
lon_0 = chivo_grid_EchoTop.origin_longitude['data']
lat_0 = chivo_grid_EchoTop.origin_latitude['data']
...
lon_0 = chivo_grid_EchoTop.origin_longitude['data']
lat_0 = chivo_grid_EchoTop.origin_latitude['data']
chivo_lons, chivo_lats = chivo_grid_EchoTop.get_point_longitude_latitude()

x, y = np.meshgrid(0.001*chivo_grid_EchoTop.x['data'],  0.001*chivo_grid_EchoTop.y['data'])
R = np.sqrt(x ** 2 + y ** 2)
...
# cs = m.pcolormesh(chivo_lons, chivo_lats, RainRate_Accummulation, vmin=1, vmax=100, latlon=True,
#         cmap=pyart.graph.cm.LangRainbow12, norm=colors.LogNorm()) #  cmocean.cm.rain
# m.colorbar(cs, label='Rain Accumulation (mm)')

EchoTop18_density = EchoTop_18/14
EchoTop14_density = EchoTop_14/14 

EchoTop18_density = np.ma.masked_where( EchoTop18_density < 0.001, EchoTop18_density)
EchoTop14_density = np.ma.masked_where( EchoTop14_density < 0.001, EchoTop14_density)

EchoTop18_density = np.ma.masked_where( R > 130, EchoTop18_density)
EchoTop14_density = np.ma.masked_where( R > 130, EchoTop14_density)

cs = m.pcolormesh(chivo_lons, chivo_lats, EchoTop14_density, vmin=0, vmax=0.2, latlon=True,
        cmap='winter',) # norm=colors.LogNorm()) #  cmocean.cm.rain
m.colorbar(cs, label='Echo Top 14 km (density)')

# Hail_Accumulation = np.ma.masked_where(Hail_Accumulation <  1, Hail_Accumulation)
# cs = m.scatter(chivo_lons, chivo_lats, s = 1 ,c = Hail_Accumulation, marker = 'o',
#           vmin=0, vmax=1, latlon=True, cmap=cmocean.cm.rain, alpha = 0.2)
