...
hail_counts = T.hail_counts;


index_toFilter = hail_counts > 3; %& max_reflectivity > 18 & max_reflectivity < 70;
...
for i = start_date:end_date
    hail_day = hail_counts(i < time_UTC & time_UTC < i + 1 & index_toFilter);
...
%     minZdr_day = min_Zdr(i < time_UTC & time_UTC < i + 1 & index_toFilter);
    ...
    days(k) = i;
    hail_counts_day(k) = nansum(hail_day);
...
hold on 
scatter(days, hail_counts_day)
datetick('x', 'dd')
ylim([0 10000])
xlim([datenum(start_date) datenum(end_date)])
ylabel('Hail counts per day')
