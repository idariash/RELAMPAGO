...
def Zdr_correction(radar, Zdr_bias):
    radar.fields['corrected_differential_reflectivity']['data'] = radar.fields['corrected_differential_reflectivity']['data'] - Zdr_bias
    
def azimuth_correction(radar, azimuth_error):
    radar.azimuth['data'] = radar.azimuth['data'] - azimuth_error

chivo = '/net/denali/storage2/radar2/tmp/Ivan/Field_Campaigns/Relampago/Analisis/Attenuation/NetCDF/DROPS/cfrad.20190126_053009.397_to_20190126_053642.486_col-radar_PPINEARLT360_SUR.nc' 
...
csapr = '/net/denali/storage2/radar2/tmp/Ivan/Field_Campaigns/Relampago/Analisis/Attenuation/NetCDF/DROPS/corcsapr2cfrppiM1.a1.20190126.053003.nc'
#'/net/denali/storage2/radar2/tmp/Ivan/Field_Campaigns/Relampago/Analisis/Attenuation/NetCDF/DROPS/corcsapr2cfrppiM1.a1.20190125.193003.nc'
#'/net/denali/storage2/radar2/tmp/Ivan/Field_Campaigns/Relampago/Analisis/Attenuation/NetCDF/CSAPR/corcsapr2cfrppiM1.a1.20190125.193003.nc'
...
#%% filter out gates with corrected_differential_reflectivity > 100 from both radars
gatefilter_CHIVO = pyart.filters.GateFilter(CHIVO)
gatefilter_CHIVO.exclude_transition()
gatefilter_CHIVO.exclude_outside('corrected_reflectivity', 10, 100)
gatefilter_CHIVO.exclude_outside('corrected_cross_correlation_ratio', 0.95, 1)

gatefilter_CSAPR = pyart.filters.GateFilter(CSAPR)
gatefilter_CSAPR.exclude_transition()
gatefilter_CSAPR.exclude_outside('corrected_reflectivity', 10, 100)
#gatefilter_CSAPR.exclude_outside('corrected_cross_correlation_ratio', 0.7, 1)
#60 = CSAPR.instrument_parameters['nyquist_corrected_reflectivity']['data'][0]
#60 = CHIVO.instrument_parameters['nyquist_corrected_reflectivity']['data'][0]
...
grids = pyart.map.grid_from_radars(
        ...
        fields=['corrected_reflectivity', 'corrected_differential_reflectivity'], gridding_algo= 'map_gates_to_grid', # 'map_to_grid', # 
...
# panel 1, basemap, radar corrected_differential_reflectivity and NARR overlay
...
# panel 1, basemap, radar corrected_reflectivity and NARR overlay
#ax1 = fig.add_axes(map_panel_axes)
#display.plot_basemap(lon_lines = np.arange(-104, -93, 1) )
display.plot_grid('corrected_differential_reflectivity', level=level, 
                  vmin= -2, vmax= 6, 
